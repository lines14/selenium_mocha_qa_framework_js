...
const X = 'admin_users'; // 'regular_users' 'admin_users'
...
describe('Validate user files:', function () {
    it('X.json contains array of users. User must have id, name and may have adress', function () {
        chai.expect(jsonFile[0]).to.deep.include.all.keys(['id', 'name']);
    });
    it('X.yaml contains array of users. User must have id, name and may have adress', function () {
